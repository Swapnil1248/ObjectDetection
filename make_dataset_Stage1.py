...
"""
    this program is written to generate the pickle file for Stage 1 of the OCR pipeline.
    It generates files that combines the text data and non-text data in
    ratio : 70:15:15 for Training:Validation:Testing
"""
...
def create_dataset(valid_dir_path, invalid_dir_path):
    ...
    # generating the data for the INVALID INPUT in scale - train:validate:test - 70:15:15
    ...
    final_val_set_x = np.concatenate((final_val_set_x,np_invalid_data_training[int(.7 * leng2): int(.85 * leng2)]),axis=0)
    final_val_set_y = np.concatenate((final_val_set_y,np_invalid_y_training[int(.7 * leng2): int(.85 * leng2)]),axis=0)
    ...
    #Concatenate the data for the valid dataset (dataset containing images with text) and invalid dataset(dataset containing images without text).
